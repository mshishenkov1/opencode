...
Ревизия 1.1, 2026-08-20 (базовая ревизия 1 — 2026-08-19). Ветка `corp/i4-sso-connectors`.
База — upstream `anomalyco/opencode` v1.17.9.
...
- **S-A11a. Построчный контракт вывода `corp status`.** Формулировки строк — часть контракта, а не
  ...
  | ключ есть, `GET /api/me` успешен | `Пользователь: <email> <user_id>`, далее `Тип ключа: <key_kind>` (если `key_kind` пришёл) |
