...
Ревизия 1.7, 2026-08-21 (ревизия 1.6 — 2026-08-21, ревизия 1.5 — 2026-08-21,
...
- **S-V14. Устойчивость разбора каталога** *(ревизия 1.7)*. Правило закрывает класс дефектов
  ...
  1. **Конверт.** Обязательны `version` (строка) и `servers` (массив). Тело не JSON-объект, отсутствие
     `version`, `servers` не массив → `hub_invalid_response`, кэш **не** перезаписывается, витрина
     ведёт себя как при недоступном Hub (S-V3).
  ...
  5. **Диагностика.** Отброшенные карточки не исчезают молча: `GET /corp/catalog` возвращает
     ...
     в лог не попадают.
  6. **Область действия.** Те же правила поэлементно применяются к `GET /api/me/connections`: ядро
...
- **S-Q2. Клиент Hub.** Модульные тесты `packages/opencode/src/corp/hub.test.ts` с моками HTTP (без сети):
  ...
  принимаются; нарушенный конверт (нет `version`, `servers` не массив) даёт `hub_invalid_response`;
  элемент `GET /api/me/connections` без `alias` отбрасывается поэлементно.
