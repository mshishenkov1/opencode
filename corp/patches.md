...
| `packages/desktop/electron.vite.config.ts` | S-B10, S-B11 | Канал `magnit` (в него же переводится `latest`), ошибка на неизвестном значении вместо отката в `dev`; build-константа `OPENCODE_CORP_UPDATE_URL` из `CORP_DESKTOP_UPDATE_URL` для главного процесса | `bfc3ce9b1b` |
