...
| `packages/opencode/src/config/config.ts` | S-C3, S-C4a, S-C9 | Мерж корпоративного слоя умолчаний до цикла `wellknown`, чтобы у слоя был наименьший приоритет; подмена адреса модели переменной `OPENCODE_CORP_MODEL_BASE_URL` внутри корп-слоя до слияния — иначе переменная перебивала бы пользовательские слои (BUG-I4-004); однократное предупреждение в лог о конфликтах личного конфига при старте (BUG-I4-006) | `06f3cada4e`, `COMMIT1`, `COMMIT3` |
| `packages/opencode/src/provider/provider.ts` | S-C4b, S-C9 | Проверка перед выдачей модели: без записи `magnit_prod` в auth-store запрос к LiteLLM не отправляется вовсе (BUG-I4-007), а выключенный личным конфигом провайдер называется по имени вместо upstream-ошибки «модель не найдена» (BUG-I4-006) | `COMMIT2`, `COMMIT3` |
| `packages/opencode/src/server/routes/instance/httpapi/middleware/error.ts` | S-C4b, S-C9 | Корп-ошибка обращения к модели отдаётся с причиной, а не как «Unexpected server error» (BUG-I4-007, BUG-I4-006) | `COMMIT2`, `COMMIT3` |
| `packages/opencode/src/cli/error.ts` | S-C4b, S-C9 | Текст причины, действия и подсказки `opencode corp login` в неинтерактивном режиме; код выхода 1 (BUG-I4-007, BUG-I4-006) | `COMMIT2`, `COMMIT3` |
