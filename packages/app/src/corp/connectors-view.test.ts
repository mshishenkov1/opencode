...
describe("витрина и страница — признак «Подключено» и объяснение ошибки (AC-177, AC-172, AC-223, AC-224)", () => {
  ...
  test("AC-224: там, где колонки нет, признак подключения — бейдж рядом с именем, а строка состояния досталась ошибкам", () => {
    // ПЕРЕНАЦЕЛЕН решением заказчика от 31.08. Прежняя формулировка требовала на странице подпись
    // состояния С ГАЛОЧКОЙ у подключённого коннектора. Заказчик, глядя на собранное приложение,
    // назвал эту строку дублем: «Подключено» уже стоит бейджем рядом с именем (макет, экран 3), а
    // «Не подключено» в макете (экран 2) не написано вовсе. Строка состояния осталась ровно у тех
    // тонов, где ей есть что сказать и где её никто не дублирует, — `warning` и `danger`: честность
    // ошибок этим решением не трогается (S-V19).
    expect(page).toContain("<Show when={statusVisible(entry())}>")
    ...
    // Галочки в строке больше нет: состояние, ради которого она рисовалась, строкой не показывается.
    expect(block).not.toContain('name="check-small"')
    // Признак подключения на странице ровно один — бейдж рядом с именем.
    expect(page).toContain('<Tag>{language.t("corp.connectors.connected")}</Tag>')
    // Бейджем `Tag` на странице остались только «Подключено», «устаревший» и признак успеха ПРЯМОГО
    // подключения внутри блока подключения (S-D13 п.7); в строке состояния бейджа нет.
