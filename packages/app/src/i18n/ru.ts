export const dict = {
  ...
  // Тот же текст для карточки `mode: "facade"` (решение заказчика от 31.08, вечер). У неё за
  // адресом коннектора стоит корпоративный шлюз: и проверка токена, и рабочие запросы идут туда, а
  // не в саму целевую систему, — и общий текст выше на такой карточке был бы неправдой. Слова
  // «Hub» здесь нет по той же причине, что и в общем тексте (D-43, AC-275): назвать в сборке без
  // Hub то, чего в ней нет, нельзя, а фасадная карточка с разобранным `upstream` подключается и там.
  "corp.connect.storageFacade":
    "Токен сохраняется на этой машине в файле с правами 0600 и уходит в корпоративный шлюз — через него коннектор работает с системой",
  ...
  // Отказ роута `direct_unavailable` (S-V25 п.1): каталог назвал способ подключения токеном, но не
  // описал, куда этот токен отправлять. Текст говорит ровно то, что известно, и называет, кому это
  // чинить, — сочинять «попробуйте ещё раз» здесь нечестно: повтор ничего не изменит.
  "corp.connect.directUnavailable":
    "Подключиться этим токеном нельзя: в карточке коннектора не описано, куда его отправлять. Сообщите владельцу коннектора",
