 ...
 * пустоте `MCP`. Прежняя деградация `type` → `owner` → пустая ячейка **отменена** ревизией 1.14:
 * имя владельца — другой факт, и в колонке типа оно неверно. Правило одно на обе оболочки — как
 * `CorpStatus.compute` (S-Q9): TUI и Desktop берут значение отсюда и сами его не выводят, поэтому
 * разойтись не могут.
 *
