...
describe("S-Q14 — секрет не покидает хранилище (AC-326)", () => {
  ...
  /**
   * MIN-A: харнесс `GET /config` (`test/fixture/corp-direct-harness.ts`, `configHandlers.get`) —
   * НЕ обработчик продукта, а его отдельная копия (см. комментарий над `configHandlers` в
   * `corp-direct-harness.ts`: типизация `HttpApiBuilder.group` ломается на `update`, если
   * подключить группу `"config"` целиком). Продуктовый обработчик — `handlers/config.ts`
   * (`export const configHandlers`), его тело `get` защищено этим файлом (S-Q14, AC-326) ТОЛЬКО
   * если оно совпадает с копией харнесса дословно: утечка, внесённая в продуктовое тело `get`
   * (лишнее поле поверх `configSvc.get()`), в харнессе не появится — весь сьют выше проверяет
   * КОПИЮ, а не продукт, и останется зелёным на дырявой реализации.
   *
   * Правки продукта здесь запрещены test-agent'у (`.claude/agents/test-agent.md`); dev предложил
   * продуктовую правку на ~2 строки — вынести тело `get` в экспортируемый именованный эффект,
   * чтобы харнесс монтировал ПРОДУКТОВУЮ функцию, а не свою копию (снимет этот тест целиком за
   * ненадобностью). До тех пор — утверждение по СРЕЗУ ИСХОДНИКА: тело `get` в обоих файлах
   * извлекается тем же регулярным выражением и сравнивается ПОСИМВОЛЬНО (не считая имени спана
   * `Effect.fn`, оно намеренно разное: `ConfigHttpApi.get` в продукте, `TestConfigHttpApi.get` в
   * харнессе). Инъекция, дописывающая поле к продуктовому `get` (например, секрет из
   * `configSvc.get()` без фильтрации, или лишний ключ рядом с `return`), эту проверку красит —
   * даже если весь остальной сьют выше её не заметит.
   */
  Harness.it.live(
    "MIN-A: тело GET /config харнесса дословно совпадает с телом GET /config продукта (иначе утечка в продукте не поймана этим файлом)",
    () =>
      Effect.gen(function* () {
        const fs = yield* Effect.promise(() => import("fs/promises"))
        const path = yield* Effect.promise(() => import("path"))
        const repoRoot = path.default.resolve(import.meta.dirname, "../../../..")

        const productPath = path.default.join(
          repoRoot,
          "packages/opencode/src/server/routes/instance/httpapi/handlers/config.ts",
        )
        const harnessPath = path.default.join(repoRoot, "packages/opencode/test/fixture/corp-direct-harness.ts")

        const [productSrc, harnessSrc] = yield* Effect.promise(() =>
          Promise.all([fs.default.readFile(productPath, "utf8"), fs.default.readFile(harnessPath, "utf8")]),
        )

        const GET_BODY_PATTERN = /const get = Effect\.fn\("[^"]*"\)\(function\* \(\) \{([\s\S]*?)\}\)/
        const productMatch = productSrc.match(GET_BODY_PATTERN)
        const harnessMatch = harnessSrc.match(GET_BODY_PATTERN)
        // Обе стороны обязаны найтись — иначе тест молчаливо ничего не проверяет (форма кода ушла
        // вперёд регулярного выражения, а не «обработчика get больше нет»).
        expect(productMatch).not.toBeNull()
        expect(harnessMatch).not.toBeNull()

        const productBody = productMatch![1].trim()
        const harnessBody = harnessMatch![1].trim()
        expect(harnessBody).toBe(productBody)
        // Дополнительно: тело — голый passthrough без фильтрации ("return yield* configSvc.get()"
        // и ничего больше), а не просто «одинаковое с харнессом» — эта строка ловит и инъекцию,
        // которая дописала бы утечку в ОБА файла синхронно (что срез-сравнение выше не поймает).
        expect(productBody).toBe("return yield* configSvc.get()")
      }),
  )
})
