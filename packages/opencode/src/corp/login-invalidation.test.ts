  ...
  test(
    ...
    async () => {
      ...
      try {
        ...
        // ПЕРЕНАЦЕЛЕНО решением заказчика от 31.08: «выход убирает и ключ, и модель». До входа
        // корп-провайдера в списке НЕТ вовсе — прежде он стоял там с пустым ключом и предлагал
        // выбрать модель, обратиться к которой нечем. Форсирующий смысл шага сохранён: состояние
        // инстанса построено, и следующая проверка отличает «построено заново» от «не строилось».
        expect(before, "провайдер magnit_prod показан в списке до входа — ключа ещё нет").toBeUndefined()
